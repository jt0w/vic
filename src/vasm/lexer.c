...
TK_Map TK_MAP[] = {
    {TK_ERR, "TK_ERR"},         {TK_INT_LIT, "TK_INT_LIT"},
    {TK_LIT, "TK_LIT"},         {TK_PUSH, "TK_PUSH"},
    {TK_POP, "TK_POP"},         {TK_ADD, "TK_ADD"},
    {TK_SUB, "TK_SUB"},         {TK_MULT, "TK_MULT"},
    {TK_DIV, "TK_DIV"},         {TK_MOD, "TK_MOD"},
    {TK_JMP, "TK_JMP"},         {TK_JZ, "TK_JZ"},
    {TK_JNZ, "TK_JNZ"},         {TK_NOP, "TK_NOP"},
    {TK_DUP, "TK_DUP"},         {TK_COLON, "TK_COLON"},
    {TK_EQ, "TK_EQ"},           {TK_SEMICOLON, "TK_SEMICOLON"},
    {TK_PERCENT, "TK_PERCENT"}, {TK_ALLOC, "TK_ALLOC"},
    {TK_WRITE, "TK_WRITE"},     {TK_READ, "TK_READ"},
    {TK_CALL, "TK_CALL"},       {TK_RET, "TK_RET"},
    {TK_NATIVE, "TK_NATIVE"},   {TK_DEF, "TK_DEF"},
    {TK_SWAP, "TK_SWAP"},       {TK_CHAR, "TK_CHAR"},
    {TK_STRING, "TK_STRING"},   {TK_USE, "TK_USE"},
...
TK_Map KeyWordMap[] = {
    ...
    {TK_MOD, "mod"},   {TK_EQ, "eq"},         {TK_JMP, "jmp"},
    {TK_NOP, "nop"},   {TK_JNZ, "jnz"},       {TK_JZ, "jz"},
    {TK_DUP, "dup"},   {TK_WRITE, "write"},   {TK_ALLOC, "alloc"},
    {TK_CALL, "call"}, {TK_RET, "ret"},       {TK_SWAP, "swap"},
    {TK_READ, "read"}, {TK_NATIVE, "native"}, {TK_DEF, "def"},
    {TK_USE, "use"},
};
...
void free_token(Token t) { free(t.span.literal); }
...
bool next_token(Lexer *lexer, Token *t) {
  ...
  case '"':
    ...
    while (true) {
      ...
      if (lexer->current == '\n') {
        fprintln(stderr,
                 "%s:%zu:%zu: error: line break in single line string literal",
