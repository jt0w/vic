...
do {                                                                                         \
  ...
  if (gen->current.args.count > 0) shift(gen->current.args.items, gen->current.args.count);  \
  if (reg >= 'A' && reg <= 'Z')                                                              \
    da_push(&p, reg - 'A');                                                                  \
  else {                                                                                     \
...
do  {                                                                         \
  if (gen->current.args.items[1].kind == TK_INT_LIT) {                        \
    ...
    push(OP_LOAD);                                                            \
    push(REG_Z);                                                              \
    da_push(&p, strtoull(gen->current.args.items[1].span.literal, NULL, 10)); \
    push((x));                                                                \
    ARG_REG();                                                                \
    push(REG_Z);                                                              \
    gen_consume(gen);                                                         \
...
Program gen_parse_expr(Gen *gen) {
  ...
  switch (gen->current.kind) {
    ...
    case EK_INT: 
    case EK_LIT: {
      fprintf(stderr, "FATAL ERROR: This should never happen please contact a maitainer (%s:%d)", __FILE__, __LINE__);
      exit(1);
    }
  }
  gen_consume(gen);
